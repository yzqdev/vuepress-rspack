...
export const dev = async (
  ...
  // console.log(util.inspect(webpackConfig.module?.rules, { showHidden: false, depth: 4, colors: true }))
  ...
  const [, close] = await Promise.all([
    ...
    new Promise<() => Promise<void>>((resolve, reject) => {
      ...
      compiler.hooks.done.tap('vuepress-dev', ({ endTime, startTime }) => {
        ...
        const url = `http://${serverConfig.host === '0.0.0.0' ? 'localhost' : serverConfig.host
